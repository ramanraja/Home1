...
int HttpPoster::post (const char *payload){
  post(payload, pC->data_prod_url);
}

/**
    Sends a Json payload
*/
int HttpPoster::post (const char *payload, const char *url){
  if (!pW->isConnected()) {
...
int HttpPoster::get (const char *url){
  ...
  SERIAL_PRINT("HttpPoster: sending url string to Gateway: ");
  SERIAL_PRINTLN(url);
  ...
  //http_client.addHeader("Content-Type", "text/plain; charset=utf-8");
